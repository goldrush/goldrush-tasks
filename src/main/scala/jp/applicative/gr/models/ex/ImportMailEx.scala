...
object ImportMailEx {
  ...
  def findBizOfferTargets(last_id:Long, days: Int) = ImportMail.findAllBy(sqls.eq(im.deleted, 0)
  ...
  def findBpMemberTargets(last_id:Long, days: Int) = ImportMail.findAllBy(sqls.eq(im.deleted, 0)
      .and.eq(im.pluralFlg, 0)
      .and.le(im.id, last_id)
      .and.eq(im.bpMemberFlg, 1)
      .and.gt(im.createdAt, (new DateTime(DateTimeZone.UTC)).minusDays(days)))

  def findAllByPage(page: Int) = ImportMail.findAllBy(sqls.eq(im.deleted, 0)
      .orderBy(im.id)
      .limit(1000)
      .offset(page * 1000))
